...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
        ...
        if (empty($error_msg)) {
            $upd = $pdo->prepare("UPDATE courses SET 
                category_id = ?, title = ?, slug = ?, duration = ?, study_mode = ?, 
                image = ?, short_desc = ?, full_desc = ?, what_you_learn = ?, how_we_teach = ?, 
                career_roles = ?, eligibility = ?, certification = ?, is_featured = ?, status = ?
                WHERE id = ?");
            $upd->execute([
                $category_id, $title, $slug, $duration, $study_mode,
                $image_path, $short_desc, $full_desc, $what_you_learn, $how_we_teach,
                $career_roles, $eligibility, $certification, $is_featured, $status,
                $course_id
            ]);
...
<div class="mb-4">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb small mb-2">
            ...
            <li class="breadcrumb-item active text-dark fw-semibold" aria-current="page">Edit: <?= htmlspecialchars($course['title']) ?></li>
        </ol>
    </nav>
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2">
        <div>
            <h4 class="fw-bold text-dark mb-0">Edit Course Details</h4>
            <p class="text-muted small mb-0">Update curriculum, rich text description, practical points, and specifications.</p>
        </div>
        <div class="d-flex gap-2">
            <a href="../course-detail.php?slug=<?= htmlspecialchars($course['slug']) ?>" target="_blank" class="btn btn-outline-primary btn-sm d-flex align-items-center gap-2">
...
<!-- ================= MAIN FORM ================= -->
<form method="POST" action="course-edit.php?id=<?= $course['id'] ?>" enctype="multipart/form-data" id="courseForm">
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            ...
            <div class="card border-0 shadow-sm rounded-3 mb-4">
                ...
                <div class="card-body p-4">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="title" id="courseTitle" class="form-control" placeholder="e.g. Professional Course in Air Hostess & Cabin Crew" value="<?= htmlspecialchars($_POST['title'] ?? $course['title']) ?>" required>
                    </div>

                    <!-- SLUG -->
                    <div class="mb-3">
                        <label class="form-label fw-bold text-dark small">URL Slug <span class="text-muted fw-normal">(Unique web address)</span></label>
                        <div class="input-group">
                            <span class="input-group-text bg-light text-muted small border-end-0">/course-detail.php?slug=</span>
                            <input type="text" name="slug" id="courseSlug" class="form-control border-start-0" placeholder="professional-course-in-air-hostess" value="<?= htmlspecialchars($_POST['slug'] ?? $course['slug']) ?>">
                        </div>
                    </div>
                    ...
                    <div class="mb-0">
                        ...
                        <textarea name="short_desc" class="form-control" rows="2" placeholder="Brief summary shown on course listing cards and hero snippet..."><?= htmlspecialchars($_POST['short_desc'] ?? $course['short_desc']) ?></textarea>
                    </div>
            ...
            <div class="card border-0 shadow-sm rounded-3 mb-4">
                ...
                <div class="card-body p-4">
                    ...
                    <textarea name="full_desc" id="summernoteDesc" class="form-control"><?= htmlspecialchars($_POST['full_desc'] ?? $course['full_desc']) ?></textarea>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm rounded-3 mb-4">
                ...
                <div class="card-body p-4">
                    ...
                    <textarea name="what_you_learn" class="form-control font-monospace small" rows="6"><?= htmlspecialchars($_POST['what_you_learn'] ?? $course['what_you_learn']) ?></textarea>
            ...
            <div class="card border-0 shadow-sm rounded-3 mb-4">
                ...
                <div class="card-body p-4">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="how_we_teach" class="form-control font-monospace small" rows="4"><?= htmlspecialchars($_POST['how_we_teach'] ?? $course['how_we_teach']) ?></textarea>
                    ...
                    <div class="mb-0">
                        ...
                        <input type="text" name="career_roles" class="form-control" placeholder="Air Hostess, Cabin Crew, Flight Attendant, Ground Staff, VIP Lounge Host" value="<?= htmlspecialchars($_POST['career_roles'] ?? $course['career_roles']) ?>">
                ...
                <div class="card-body p-3">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status" class="form-select form-select-sm">
                            <option value="active" <?= (($_POST['status'] ?? $course['status']) === 'active') ? 'selected' : '' ?>>Active &bull; Visible on Website</option>
                            <option value="inactive" <?= (($_POST['status'] ?? $course['status']) === 'inactive') ? 'selected' : '' ?>>Inactive &bull; Hidden Draft</option>
                        </select>
                    </div>

                    <!-- FEATURED TOGGLE -->
                    <div class="mb-3 form-check form-switch">
                        <input class="form-check-input" type="checkbox" name="is_featured" value="1" id="featuredSwitch" <?= (!empty($_POST['is_featured'] ?? $course['is_featured'])) ? 'checked' : '' ?>>
                    ...
                    <div class="d-grid gap-2">
                        <button type="submit" class="btn btn-primary d-flex align-items-center justify-content-center gap-2 py-2">
                            ...
                            <span class="fw-semibold">Update Course</span>
                        </button>
                ...
                <div class="card-body p-3">
                    ...
                    <div class="mb-3">
                        <div class="d-flex justify-content-between align-items-center mb-1">
                            ...
                            <a href="courses.php" class="small text-decoration-none" style="font-size: 11.5px;">Manage Categories</a>
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="duration" class="form-control form-control-sm" placeholder="e.g. 12 Months / 6 Months" value="<?= htmlspecialchars($_POST['duration'] ?? $course['duration']) ?>">
                    </div>

                    <!-- STUDY MODE -->
                    <div class="mb-3">
                        <label class="form-label fw-semibold text-secondary small">Study Mode</label>
                        <input type="text" name="study_mode" class="form-control form-control-sm" placeholder="e.g. Classroom & Practical Labs" value="<?= htmlspecialchars($_POST['study_mode'] ?? $course['study_mode']) ?>">
                    </div>

                    <!-- ELIGIBILITY -->
                    <div class="mb-3">
                        <label class="form-label fw-semibold text-secondary small">Eligibility Criteria</label>
                        <input type="text" name="eligibility" class="form-control form-control-sm" placeholder="e.g. 10+2 (12th Pass) in any stream" value="<?= htmlspecialchars($_POST['eligibility'] ?? $course['eligibility']) ?>">
                    </div>

                    <!-- CERTIFICATION -->
                    <div class="mb-0">
                        <label class="form-label fw-semibold text-secondary small">Certification Title</label>
                        <input type="text" name="certification" class="form-control form-control-sm" placeholder="e.g. Industry Recognized Diploma" value="<?= htmlspecialchars($_POST['certification'] ?? $course['certification']) ?>">
                    </div>
                ...
                <div class="card-body p-3 text-center">
                    ...
                    <div class="preview-box mb-3" id="imagePreviewBox">
                        <?php if (!empty($course['image'])): ?>
                            <img src="../<?= htmlspecialchars($course['image']) ?>" alt="Current Thumbnail" id="previewImg">
                        <?php else: ?>
                    ...
                    <div class="text-start">
                        ...
                        <div class="form-text small mt-1 text-muted" style="font-size: 11.5px;">
                            Leave empty to keep existing thumbnail. Max 2MB.
                        </div>
                    </div>
